...
export class Images extends React.Component {
    ...
    loadImages(Id) {
      ...
      var link = '';
      !this.props.isTalent ? link = 'getEmployer' : link = 'get';
      $.ajax({
        url: 'http://localhost:60290/profile/profile/' + link + 'ProfileImage/?id=' + Id,
            headers: {
            ...
            success: function (res) {
                ...
                if (typeof(res.employerProfile) != undefined && res.employerProfile.length > 0) {
                    for (var i = 0; i < res.employerProfile.length; i++) {
                        imageSrcArr.push("http://localhost:60290/profile/profile/" + link + "ProfileImages/?Id=" + res.employerProfile[i].fileName);
                        imageIdArr.push(res.employerProfile[i].id);
                        selectedFileArr.push("");
                    }
                }
              if (typeof (res.talentProfile) != undefined && res.talentProfile.length > 0) {
                for (var i = 0; i < res.talentProfile.length; i++) {
                  imageSrcArr.push("http://localhost:60290/profile/profile/" + link + "ProfileImages/?Id=" + res.talentProfile[i].fileName);
                  imageIdArr.push(res.talentProfile[i].id);
                  selectedFileArr.push("");
                }
              }
              var x = 0; !this.props.isTalent ? x = res.employerProfile.length : x = res.talentProfile.length;
                this.setState({
    ...
    fileUploadHandler(Id) {
        ...
        var cookies = Cookies.get('talentAuthToken');
        var link = '';
        !this.props.isTalent ? link = 'addEmployer' : link = 'addTalent';
      console.log("d", data);
        $.ajax({
            url: 'http://localhost:60290/profile/profile/' + link + 'ProfileImages',
            headers: {
                'Authorization': 'Bearer ' + cookies
            },
            type: "POST",
    ...
    render() {
      ...
      var name = ''; !this.props.isTalent ? name = 'your company' : name = 'yourself';
        return (
            <div className="row">
                <div className="four wide column">
                    <h3>Images</h3>
              <div className="tooltip">Upload images of {name} here</div>
                </div>
