    ...
    public class ProfileController : Controller
    {
        ...
        public async Task<IActionResult> GetProfile()
        {
            try
            {
                ...
                return Json(new { Success = true, talent = user });
            }
            catch (Exception e)
            {
        ...
        public async Task<IActionResult> GetLanguages()
        {
            var userId = _userAppContext.CurrentUserId;
            var user = await _userRepository.GetByIdAsync(userId);
            return Json(new { userLanguage = user.Languages });
        }
        ...
        public async Task<ActionResult> UpdateLanguage([FromBody] AddLanguageViewModel language)
        {
            try
            {               
                var lan = await _userLanguageRepository.GetByIdAsync(language.Id);
                lan.Language = language.Name;
                lan.LanguageLevel = language.Level;
                await _userLanguageRepository.Update(lan);
        ...
        public async Task<ActionResult> DeleteLanguage([FromBody] AddLanguageViewModel language)
        {
            ...
            {
                var lan = await _userLanguageRepository.GetByIdAsync(language.Id);                
                await _userLanguageRepository.Delete(lan);
                return Json(new { Success = true });
            }
            catch (Exception e)
            {
        ...
        public async Task<IActionResult> GetSkills()
        {
            var userId = _userAppContext.CurrentUserId;
            var user = await _userRepository.GetByIdAsync(userId);
            return Json(new { userSkills = user.Skills });
        }
        ...
        public ActionResult getProfileImage(string Id)
        {
            ...
            if( profileUrl == null)
            {
                return Json(new { Success = false, message = "Profile Image" });
            }
        ...
        public async Task<ActionResult> UpdateProfilePhoto()
        {
            ...
            return Json(new { Success = false });
            //var x = await _profileService.UpdateTalentPhoto(Id, file);
            //    return Json(new { success = x });          
        }
        ...
        public async Task<IActionResult> GetTalentProfile(String id = "")
        {
            try
            {
                ...
                return Json(new { Success = true, talent = userProfile });
            }
            catch (Exception e)
            {
                return Json(new { Success = false, message = e });
        ...
        public async Task<IActionResult> GetTalentListAsync()
        {
            try
            {
                 var result = await _profileService.GetATalentList();
                //var result = await _profileService.GetEmployerList();
                return Json(new { Success = true, Data = result });
            }
